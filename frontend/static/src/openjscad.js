...
OpenJsCad.Viewer.prototype = {
  ...
  createRenderer: function(bool_noWebGL) {
    var Renderer =
      this.webGLAvailable() && !bool_noWebGL
        ? THREE.WebGLRenderer
  ...
  setCsg: function(csg, resetZoom) {
    ...
    var colorMeshes = [].concat(res.colorMesh).map(function(mesh) {
  ...
  parseSizeParams: function() {
    ...
    var hUnit =
      typeof this.size.height == "string"
        ? this.size.height.match(/^(\d+(?:\.\d+)?)(.*)$/)[2]
        : ""
    // whether unit scales on win resize
    var isDynUnit =
      winResizeUnits.indexOf(wUnit) != -1 || winResizeUnits.indexOf(hUnit) != -1
...
OpenJsCad.makeAbsoluteUrl = function(url, baseurl) {
  if (!url.match(/^[a-z]+\:/i)) {
    ...
    if (url[0] == "/") {
      ...
      url = (baseurl.replace(re, "") + "/" + url.replace(re, "")).replace(
        /[^\/]+\/\.\.\//g,
        ""
      )
    }
...
OpenJsCad.checkResult = function(result) {
      ...
      if (result.length < 1) {
        ...
        result.forEach(function(resultelement) {
          if (!("data" in resultelement)) {
            ...
            if (
              resultelement.data instanceof CSG ||
              resultelement.data instanceof CAG
            ) {
              // ok
            } else {
              ok = false
  ...
  if (!ok) {
    throw new Error(
      "Your main() function does not return valid data. It should return one of the following: a CSG object, a CAG object, an array of CSG/CAG objects, or an array of objects: [{name:, caption:, data:}, ...] where data contains a CSG or CAG object."
    )
  }
}
...
OpenJsCad.parseJsCadScriptASync = function(
  ...
  var baselibraries = ["/src/csg.js", "/src/openjscad.js"]
  ...
  if (typeof options["openJsCadPath"] != "undefined") {
    ...
    // on openjscadurl
    openjscadurl =
      OpenJsCad.makeAbsoluteUrl(options["openJsCadPath"], baseurl) + "/"
  }
  ...
  if (!window.Worker)
    throw new Error(
      "Your browser doesn't support Web Workers. Please try the Chrome browser instead."
    )
...
OpenJsCad.Processor.prototype = {
  ...
  setRenderedObjects: function(obj) {
    ...
    while (this.renderedElementDropdown.options.length > 0)
      this.renderedElementDropdown.options.remove(0)
    ...
    this.renderedElementDropdown.style.display =
      obj.length >= 2 ? "inline" : "none"
  ...
  setSelectedObjectIndex: function(index) {
    ...
    if (obj !== null) {
      ...
      this.isFirstRender_ =
        typeof this.isFirstRender_ == "undefined" ? true : false
      ...
      this.supportedFormatsForCurrentObject().forEach(function(format) {
        ...
        option.appendChild(
          document.createTextNode(this.formatInfo(format).displayName)
        )
  ...
  enableItems: function() {
    ...
    this.formatDropdown.style.display =
      !this.hasOutputFile && this.hasValidCurrentObject ? "inline" : "none"
    ...
    this.parametersdiv.style.display =
      this.paramControls.length > 0 ? "block" : "none"
      ...
      this.worker = OpenJsCad.parseJsCadScriptASync(
        ...
        function(err, obj) {
          ...
          if (err) {
            ...
            that.statusspan.innerHTML =
              "Ready." +
              (that.options.verbose ? "  Rendered in " + elapsed + "ms" : "")
          }
          that.enableItems()
      ...
      if (type == "text" || type == "int" || type == "float") {
        ...
        if (initial !== undefined) {
          if (typeof initial != "boolean") {
            throw new Error(
              errorprefix +
                "initial/default of type 'bool' has to be boolean (true/false)"
            )
          }
